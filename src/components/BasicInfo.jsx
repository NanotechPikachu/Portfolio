export default function BasicInfo() {
  return (
    <>
    <h1 className="font-bold text-center text-2xl md:text-3xl dark:text-white text-black mb-4">Hi, I&apos;m NanotechPikachu!</h1>
    <p className="font-mono text-sm dark:text-zinc-50 text-black md:text-base text-center"><i>A developer with passion!</i></p>
    <div className="mt-6 mr-2 ml-2 border dark:border-teal-100 border-teal-200 dark:bg-[#2E3532] bg-slate-100 border">
    <p className="font-sans md:text-center md:text-base text-sm dark:text-zinc-50 text-black mr-3 ml-3 mt-2 mb-2">
I&apos;m a <b>Computer Science</b> student and an ardent fan of programming. A developer and an author by passion. Love to make webpages and many other things!
    </p>
    </div>
    </>
  )
}